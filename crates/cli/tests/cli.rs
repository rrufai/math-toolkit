...
fn run_solve(args: &[&str]) -> (String, String, i32) {
    let out = solve_bin().args(args).output().expect("failed to run solve binary");
    let stdout = String::from_utf8_lossy(&out.stdout).into_owned();
    let stderr = String::from_utf8_lossy(&out.stderr).into_owned();
    let code   = out.status.code().unwrap_or(-1);
    (stdout, stderr, code)
...
fn test_solve_x_squared_minus_2() {
    let (out, _, code) = run_solve(&["x^2 - 2", "1", "2"]);
    assert_eq!(code, 0);
...
fn test_solve_no_sign_change() {
    let (_, err, code) = run_solve(&["x^2", "1", "3"]);
    assert_ne!(code, 0);
...
fn test_solve_parse_error() {
    let (_, err, code) = run_solve(&["sin(x +", "0", "4"]);
    assert_ne!(code, 0);
...
fn test_solve_missing_bounds() {
    let (_, err, code) = run_solve(&["x^2 - 2"]);
    assert_ne!(code, 0);
    assert!(err.contains("Usage"), "err={}", err);
}

// ── integrate error cases ─────────────────────────────────────
